...
class HostGwQueryForm(forms.Form):
    ...
    snmpver = forms.ChoiceField(label=u'Версия SNMP', choices=((1, 'v.1'),(2, 'v.2c')))
...
class HostQueryForm(forms.Form):
    ...
    host = forms.CharField(max_length=255, label=u'Устройство')
    community = forms.CharField(widget=forms.PasswordInput, max_length=255, label=u'Пароль (community)', required=False)
